...
const annotateParams = params =>
  params.reduce(
    (acc, p) =>
      Array.isArray(p)
        ? acc.concat([annotateParams(p)])
        : Object.keys(p).length === 2 && p.name && p.value
        ? acc.concat(p)
        : acc.concat(formatParam(Object.keys(p)[0], Object.values(p)[0])),
    []
  ); // end reduce
  ...
  return (sql.match(/\:{1,2}\w+/g) || [])
    .map((p, i) => {
      return p === "??"
        ? { type: "id" } // identifier
        : p === "?"
        ? { type: "ph", label: "__d" + i } // placeholder
        : p.startsWith("::")
...
const formatRecords = (recs, columns) => {
  ...
  let fmap =
    recs && recs[0]
      ? recs[0].map((x, i) => {
          return Object.assign({}, columns ? { label: columns[i].label } : {}); // add column labels
        })
      : {};
