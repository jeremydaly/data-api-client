...
The **Data API Client** is a lightweight wrapper that takes the pain out of working with the Amazon Aurora Serverless Data API. The Data API makes you annotate every field value with its type, both going in and coming back, which gets old fast. This library does that for you, mapping native JavaScript types to the Data API's format and back automatically. It's basically a [DocumentClient](https://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/DynamoDB/DocumentClient.html) for the Data API. It also handles **transactions** cleanly, adds **automatic retry logic** for scale-to-zero clusters, and ships drop-in **compatibility layers** for mysql2 and pg. Point **Knex**, **Drizzle**, or **Kysely** at the Data API and keep writing the queries you already know.
